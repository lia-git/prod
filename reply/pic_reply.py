        ...
        if r.exists(key):
            ...
            line = (
                Bar(init_opts=opts.InitOpts(height="500px",width="2100px",js_host="/js/",page_title=names[ix]))
                    ...
                    .set_global_opts(title_opts=opts.TitleOpts(title=f"{names[ix]}-{cmcs[ix]}-{round(max(vals_)/cmcs[ix],2)}-{round(vals_[-1]/cmcs[ix],2)}主力趋势"),yaxis_opts=opts.AxisOpts(type_="value", min_=0,max_=max(vals_),axistick_opts=opts.AxisTickOpts(is_show=True),splitline_opts=opts.SplitLineOpts(is_show=True)))
            )
            # lines.append(line)
            cnt += 1
            page.add(line)
...
def reply_stock_main_power(name):
    ...
    line = (
        Bar(init_opts=opts.InitOpts(height="700px",width="1800px",js_host="/js/",page_title=name))
            ...
            .set_series_opts(label_opts=opts.LabelOpts(is_show=False))
            .set_global_opts(title_opts=opts.TitleOpts(title=f"{name}-{cmc}-{round(max(vals_)/cmc,2)}-{round(vals_[-1]/cmc,2)}主力趋势"),yaxis_opts=opts.AxisOpts(type_="value", min_=0,max_=max(vals_),axistick_opts=opts.AxisTickOpts(is_show=True),splitline_opts=opts.SplitLineOpts(is_show=True)))
    )
