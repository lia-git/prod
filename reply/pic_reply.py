...
def reply_stable():
    if check_():
            return
    file_table = get_stable_stock()
    wechat = WeChatPub()
    # wechat.send_markdown(content)
    to_file(file_table,f"ups/stable.xlsx")
    wechat.send_file(f"ups/stable.xlsx")
    ...
    for ix, code in enumerate(code_list):
        try:
            ...
            if r.exists(trend_key):
                ...
                cnt += 1
                page.add(line)
                file_table.append([code[2:],names[ix],cmcs[ix],ups[ix],prices[ix]])
