...
def get_all_db(flag=True):
    ...
    try:
        # 执行SQL语句
        cursor.execute(f"select  {segment}  from stock_base where stock_code not  like 'sz300%' and stock_name not like '%ST%' and last_price between 4.0 and 500 ;")
    ...
    try:
        ...
        for record in new_stocks:
            if record["stock_code"] in description_dict:
                ...
                s_r =[ "%s" for i in  range(len(record.keys()))]
